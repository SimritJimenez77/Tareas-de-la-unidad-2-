...
class MenuScreen(tk.Frame):
    def __init__(self, master):
        ...
        self.edit_menu = tk.Menu(self.menu_bar, tearoff = 0)
        self.edit_menu.add_command(label = "Enviar datos" , command = self.send)
        self.edit_menu.add_command(label = "Configuracion" , command = self.configure)
        self.edit_menu.add_command(label = "Actualizar ventana" , command = self.update)
        self.menu_bar.add_cascade(label="Opciones", menu=self.edit_menu)



        # Agregamos algunos widgets a la pantalla
        self.label = tk.Label(self, text="¡BIENVENIDO AL MENU!")
